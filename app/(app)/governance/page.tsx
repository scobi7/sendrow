...
export default async function Governance() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        title="Governance — Your Leadership and Policies"
      ...
      <div className="card mb-5">
        ...
        <p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>
          Document upload on each &ldquo;yes&rdquo; is wired to S3 in the production build.
        </p>
            ...
            return (
              ...
              >
                ...
                <span className="flex items-center gap-2">
                  {v === true && (
                    <span className="text-xs font-semibold" style={{ color: "var(--status-green)" }}>✓ In place</span>
                  )}
                  {v === false && (
                    <span className="text-xs font-semibold" style={{ color: "var(--warning)" }}>Not yet — see Gap Analysis</span>
                  )}
                  <form action={savePolicy.bind(null, p, true)}>
                    <button
                      className="btn btn-secondary px-2.5 py-1 text-xs"
                      style={v === true ? { borderColor: "var(--primary)", color: "var(--primary)" } : {}}
                    >
                      Yes
                    </button>
                  </form>
                  <form action={savePolicy.bind(null, p, false)}>
                    <button
                      className="btn btn-secondary px-2.5 py-1 text-xs"
                      style={v === false ? { borderColor: "var(--warning)", color: "var(--warning)" } : {}}
                    >
                      No
                    </button>
                  </form>
