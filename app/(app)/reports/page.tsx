      ...
      <div className="card mb-6 grid grid-cols-2 gap-4 text-center sm:grid-cols-4">
        {[["Scope 1", t.scope1], ["Scope 2", t.scope2Location], ["Scope 3", t.scope3], ["Total", t.total]].map(([label, v]) => (
          <div key={label as string}>
            ...
            <p className="text-xs text-zinc-400">tCO2e</p>
          </div>
        ))}
      </div>
        ...
        <ul className="mt-3 space-y-2 text-sm">
          {checks.map(([label, ok, detail]) => (
            <li key={label} className="flex items-center gap-3">
              <span className={`flex h-5 w-5 items-center justify-center rounded-full text-xs font-bold ${ok ? "bg-brand-100 text-brand-700" : "bg-red-100 text-red-600"}`}>{ok ? "✓" : "!"}</span>
      ...
      <div className="space-y-4">
        <div className="card flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">Audit-ready PDF: Scope 1, 2 (dual reporting), 3, methodology, and data quality notes.</p>
          </div>
        ...
        <div className="card">
          ...
          <p className="mt-1 text-sm text-zinc-500">Select your customer&rsquo;s questionnaire format to see exactly which of your numbers answers which question.</p>
          ...
          {mapping && (
            <table className="mt-4 w-full text-sm">
              <thead><tr className="border-b border-zinc-200 text-left text-xs uppercase tracking-wide text-zinc-400">
                <th className="pb-2 pr-2">Question</th><th className="pb-2">Their field</th><th className="pb-2 text-right">Your answer</th>
              </tr></thead>
              <tbody>
                {mapping.map((row, i) => (
                  <tr key={i} className="border-b border-zinc-100 align-top">
                    <td className="py-2 pr-2 text-xs font-bold text-zinc-400">{row.questionId}</td>
                    <td className="py-2 pr-4 text-zinc-700">{row.question}{row.note && <span className="block text-xs text-zinc-400">{row.note}</span>}</td>
        ...
        <div className="card flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">Every data point, its source, the emission factor used, and who entered it.</p>
          </div>
