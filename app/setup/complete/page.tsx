...
export default async function SetupComplete() {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-xl flex-col items-center justify-center bg-zinc-50 px-6 text-center">
      <Logo />
      <div className="mt-8 flex h-16 w-16 items-center justify-center rounded-full bg-brand-100 text-3xl text-brand-600">✓</div>
      ...
      <div className="mt-8 grid w-full grid-cols-3 gap-4">
        {[["📊", "GHG Inventory Report"], ["🗂️", "Questionnaire Helper"], ["🔍", "Audit Trail"]].map(([icon, title]) => (
      ...
      <Link href="/dashboard" className="btn-primary mt-10 px-8 py-3 text-base shadow-md">Go to My Dashboard →</Link>
    </main>
  );
}
