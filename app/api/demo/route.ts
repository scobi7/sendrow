...
import { uid, createUser, saveCompany, appendAudit, getUserByEmail } from "@/lib/store";
...
const DEMO_EMAIL = "demo@pacificcoastlogistics.example";

export async function GET(request: Request) {
  // Require ?key=<DEMO_SECRET> in production. Set DEMO_SECRET env var to enable; leave unset to disable.
  ...
  if (!user) {
    const company: Company = {
      ...
      utilityData: [],
      inputs: {
        fleet_gasoline_gal: 6200,
        fleet_diesel_gal: 48500,
        fleet_propane_gal: null,
        refrigerant_type: "R-410A",
        refrigerant_kg: 14,
        equipment_fuel_type: "Diesel",
        equipment_gal: 3100,
        scope2_reviewed: true,
        has_recs: true,
        rec_coverage_pct: 35,
        rec_certificate_name: "2025 Green-e REC certificate",
        qb_data_reviewed: true,
        commute_avg_miles: 24,
        commute_mode: "Drive alone",
        commute_days_in_office: 4,
        waste_landfill_tons: 86,
        waste_recycled_tons: 41,
        waste_composted_tons: 12,
        scope3_other_categories: {
          "Capital goods": "industry_average",
          "Fuel- and energy-related activities": "industry_average",
          "Downstream transportation": "na",
          "Use of sold products": "na",
          "End-of-life treatment of sold products": "na",
          "Leased assets": "na",
        },
        social_total_employees: 240,
        social_new_hires: 38,
        social_departures: 29,
        social_lost_time_injuries: 2,
        social_osha_recordables: 5,
        social_near_misses: 14,
        social_days_lost: 22,
        social_training_hours: 2900,
        social_demographics_uploaded: true,
        gov_leadership: {
          "C-Suite": { womenPct: 25, minorityPct: 25 },
          "VP/Director": { womenPct: 33, minorityPct: 30 },
          "Manager": { womenPct: 41, minorityPct: 38 },
          "Individual Contributor": { womenPct: 36, minorityPct: 52 },
        },
        gov_policies: {
          "Code of conduct": true,
          "Whistleblower policy": false,
          "Anti-bribery policy": false,
          "Data privacy policy": true,
          "Environmental policy": false,
          "Equal opportunity policy": true,
        },
        gov_ccpa_compliant: true,
        gov_public_privacy_policy: true,
        gov_data_breaches: false,
      },
      calcs: [],
    ...
    user = {
      ...
      name: "Dana Reyes",
      email: DEMO_EMAIL,
      passHash: hashPassword("greentrack-demo"),
    ...
    await appendAudit({
      ...
      field: "demo_seed",
      prev: "—",
      next: "Demo company created with simulated QuickBooks + utility data",
    });
  }
